...
- [X] Explain the difference between `Relational Databases` and `SQL`.
        ...
        the difference between `Relational Databases` and `SQL` is that `Relational Databases` is a system that manages storage, querying, and updating. 'SQL' is a language which is typically used to query the `Relational Databases`.

- [X] Why do tables need a `primary key`?

        Because it's a way to uniquely identify each entry in the database.

- [X] What is the name given to a table column that references the primary key on another table.

        These are called 'Foreign keys'. They are used for creating links between tables. Whereas a primary key is used to id rows in a table, foreign keys are used connect a record in one table to a record in a second table.

- [X] What do we need in order to have a _many to many_ relationship between two tables.

        We'll need to an intermediary table that holds 'Foreign keys' that reference the primary key on the related tables.
